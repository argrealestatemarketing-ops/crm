...
/** Combine a dialing code and a local number into one E.164-ish string. */
export function combinePhone(code: string, localNumber: string): string {
  const trimmed = localNumber.trim()
  if (!trimmed) return ""
  // If the user already typed a full international number, keep it as-is.
  if (trimmed.startsWith("+")) return trimmed.replace(/\s+/g, "")
  // Drop a leading 0 from the local part before prefixing the country code.
